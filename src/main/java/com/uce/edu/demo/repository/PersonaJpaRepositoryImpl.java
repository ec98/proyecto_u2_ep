...
//		CriteriaQuery<Persona> miQueryCompleto = myQuery.select(personaRoot).where(p1);
	...
	public List<PersonaSencilla> buscarPorApellidoSencillo(String apellido) {
		TypedQuery<PersonaSencilla> miQuery = this.entityManager.createQuery(
	...
	public List<PersonaContadorGenero> consultarCantidadPorGenero() {
		// select pers_genero, count (*) from persona group by pers_genero
		...
		TypedQuery<PersonaContadorGenero> miQuery = this.entityManager.createQuery(
